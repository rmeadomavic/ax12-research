...
the PCB, so the MT6631 GNSS core acquires zero satellites (verified: AGC at
thermal noise floor, GNSS RTC never set, blank NMEA SNR). The positions this
tool returns come from Android's WiFi/network ('fused'/'network') providers —
useful only where the device can geolocate against a known network, never in
the field. The 'nmea'/'satellites' subcommands will show sats in view (almanac
dots) with no signal. See docs/hardware/hardware-map.md.
